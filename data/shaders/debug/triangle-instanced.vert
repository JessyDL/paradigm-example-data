...
layout (location = 3) in mat4x4 inModelMat;
...
layout(std140, binding = 0) uniform GLOBAL_WORLD_VIEW_PROJECTION_MATRIX
{
	FrameData data[128];
} ubo;
...
void main() 
{
	...
	outTex = inTex;
	vec3 position = (ubo.data[0].modelMatrix * inModelMat * vec4(inPosition.xyz, 1.0)).xyz;	
	gl_Position = ubo.data[0].WVP * vec4(position, 1.0);
	
	//vec3 position = (inModelMat * vec4(inPosition.xyz, 1.0)).xyz;	
